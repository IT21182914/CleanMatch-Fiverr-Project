...
const BookService = () => {
  ...
  const updateURLParams = (newFormData) => {
    ...
    if (newFormData.scheduledDate) params.set('date', newFormData.scheduledDate);
    else params.delete('date');

    if (newFormData.scheduledTime) params.set('time', newFormData.scheduledTime);
    ...
    if (newFormData.specialInstructions) params.set('instructions', newFormData.specialInstructions);
    else params.delete('instructions');

    if (newFormData.latitude) params.set('lat', newFormData.latitude.toString());
    else params.delete('lat');

    if (newFormData.longitude) params.set('lng', newFormData.longitude.toString());
    else params.delete('lng');

    if (newFormData.locationMethod) params.set('locationMethod', newFormData.locationMethod);
  ...
  useEffect(() => {
    ...
    if (viewFromURL && ['services', 'details', 'booking', 'confirmation'].includes(viewFromURL)) {
  ...
  const fetchServices = async () => {
    try {
      ...
      if (targetServiceId) {
        const service = servicesData.find(s => s.id === parseInt(targetServiceId));
        if (service) {
