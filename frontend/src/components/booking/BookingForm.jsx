...
const BookingForm = ({
  ...
  const handleLocationDetection = async () => {
    try {
      ...
      toast.loading("Collecting your location data for better service...", {
        id: "location-collecting",
      });
      ...
      if (navigator.permissions) {
        ...
        if (permission.state === "denied") {
          toast.error(
            "Location access denied. Please enable location services or use postal code method.",
            { id: "location-collecting" }
          );
          onChange({
            target: {
      ...
      navigator.geolocation.getCurrentPosition(
        (position) => {
          ...
          setIsCollectingLocation(false);
          toast.success(
            "Location collected successfully! No need to collect again unless you move.",
            { id: "location-collecting" }
          );
        },
        (error) => {
          ...
          setIsCollectingLocation(false);
          toast.error(
            "Failed to collect location. Please use postal code method instead.",
            { id: "location-collecting" }
          );
          onChange({
            target: {
  ...
  const validateForm = () => {
    ...
    if (!formData.scheduledTime) {
      ...
      if (selectedDate.toDateString() === today.toDateString()) {
        const [selectedHour, selectedMinute] = formData.scheduledTime
          .split(":")
          .map(Number);
        const selectedTimeInMinutes = selectedHour * 60 + selectedMinute;
        const currentTimeInMinutes =
          today.getHours() * 60 + today.getMinutes() + 120; // 2 hours buffer
    ...
    if (!formData.workDuration)
      newErrors.workDuration = "Please select work duration";
    ...
    if (locationMethod === "gps") {
      ...
      if (!formData.latitude || !formData.longitude) {
        newErrors.location =
          'Please collect your GPS location by clicking "Get My Location"';
      }
      if (!formData.address)
        newErrors.address = "Full address is required with GPS method";
      if (!formData.city) newErrors.city = "City is required with GPS method";
      if (!formData.state)
        newErrors.state = "State is required with GPS method";
      ...
      <Card>
        <CardContent className="p-3 sm:p-4">
          <div className="flex flex-col sm:flex-row items-start sm:items-center space-y-3 sm:space-y-0 sm:space-x-4">
            ...
            <div className="flex flex-col items-center sm:items-end justify-center mt-3 sm:mt-0 w-full sm:w-auto">
              <button
                type="button"
                onClick={() => (window.location.href = "/customer/membership")}
                className="bg-gradient-to-r from-emerald-500 to-teal-500 hover:from-emerald-600 hover:to-teal-600 text-white text-xs sm:text-sm font-bold py-2 px-3 sm:px-4 rounded-lg shadow-md hover:shadow-lg transform hover:scale-105 transition-all duration-300 w-full sm:w-auto"
              >
                ...
                <div className="space-y-2">
                  <label className="flex items-center cursor-pointer">
                    <input
                      ...
                      onChange={(e) =>
                        handleLocationMethodChange(e.target.value)
                      }
                    ...
                    <span className="text-sm flex-1">
                      ...
                      <span className="text-gray-500 text-xs">
                        Most accurate for finding nearby cleaners + full address
                        required
                      </span>
                    </span>
                  </label>
                  <label className="flex items-center cursor-pointer">
                    ...
                    <span className="text-sm flex-1">
                      ...
                      <br />
                      <span className="text-gray-500 text-xs">
                        Enter your address details and postal code
                      </span>
                    </span>
                  </label>
                </div>
                ...
                <div className="space-y-4">
                  <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                    ...
                    <p className="text-sm text-blue-700 mb-2">
                      Click "Get My Location" to allow us to collect your GPS
                      coordinates once. You'll also need to provide your full
                      address below.
                    </p>
                    ...
                    formData.longitude ? (
                      <div className="text-sm text-green-700 font-medium bg-green-50 border border-green-200 rounded p-2">
                        ✓ GPS Location collected: {formData.latitude.toFixed(4)}
                        , {formData.longitude.toFixed(4)}
                      </div>
                    ) : gpsPermissionStatus === "denied" ? (
                      <div className="text-sm text-red-700 bg-red-50 border border-red-200 rounded p-2">
                        ✗ Location access denied. Please enable location
                        services in your browser settings or use postal code
                        method.
                      </div>
                    ) : isCollectingLocation ? (
              ...
              {locationMethod === "postal" && (
                <div className="space-y-4">
                  ...
                  <p className="text-sm text-gray-600">
                    We'll use your postal code and address details to find
              ...
              <div className="flex items-start">
                ...
                <div className="text-sm text-yellow-700">
                  ...
                  <ul className="space-y-1">
                    <li>
                      • GPS location: We collect your coordinates once + you
                      provide full address details
                    </li>
                    <li>
                      • Postal code: You provide full address details + postal
                      code for service location
                    </li>
                    <li>• Our cleaners arrive within a 2-hour window</li>
                    <li>
                      • You'll receive a confirmation call 24 hours before
                    </li>
