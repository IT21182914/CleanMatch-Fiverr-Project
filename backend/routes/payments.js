...
// @access  Private (Cleaners only)
router.get("/connect-status", auth, authorize("cleaner"), async (req, res) => {
  try {
    const cleanerResult = await query(
      "SELECT stripe_account_id FROM cleaner_profiles WHERE user_id = $1",
      [req.user.id]
...
router.post("/transfer", auth, authorize("admin"), async (req, res) => {
  try {
    const { bookingId } = req.body;
...
// @access  Private (Customers only)
router.get("/subscription", auth, authorize("customer"), async (req, res) => {
  try {
    const subscriptionResult = await query(
