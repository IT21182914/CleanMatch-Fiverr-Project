...
const testHealthCheck = async () => {
  try {
    ...
    if (error.message.includes('listen EADDRINUSE')) {
      console.log('💡 Tip: Port 5000 is already in use. Try stopping other servers or change PORT in .env');
    } else if (error.message.includes('connect ECONNREFUSED')) {
      console.log('💡 Tip: Database connection failed. Check your database configuration in .env');
    }
